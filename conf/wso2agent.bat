...
%_RUNJAVA% %JAVA_OPTS% -Dtransports.netty.conf="%CARBON_HOME%"\conf\netty-transports.yml -Dcarbon.home="%CARBON_HOME%" -Dlog4j.configuration=file:"$CARBON_HOME"\conf\log4j.properties -Djavax.net.ssl.trustStore="$CARBON_HOME"\conf\security\client-truststore.jks -cp "%CARBON_CLASSPATH%" org.wso2.carbon.identity.agent.userstore.Application %*
endlocal
:end
PAUSE
